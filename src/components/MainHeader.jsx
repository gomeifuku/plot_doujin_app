...
class CreateCharacterItem extends Component {
  ...
  textChange(event) {
    const formInput = this.state.newCharacterStates
    ...
    return (
      <div className="modalCreateCharacter">
        <Dialog
          ...
          onClose={this.props.onCancelAction}
        >
          ...
          <DialogContent>
            <div className="modalCreateCharacterForm">
              <TextField
                label="ID"
                onChange={event =>
                  this.textChange({ key: 'id', text: event.target.value })
                }
              ...
              <TextField
                label="名前"
                onChange={event =>
                  this.textChange({ key: 'name', text: event.target.value })
                }
    ...
    const list = this.props.characterList.map((component, index) => {
      return (
        <MenuItem onClick={this.handleClose} key={index}>
          ...
          <Avatar src={component.avatar[0].avatarImagePath} />
        </MenuItem>
      )
    })
    return list
    ...
    return (
      <div className="MainHeader">
        <CreateCharacterItem
          ...
          onCancelAction={e => this.toggleCreateCharacterForm(e)}
        />
        <Drawer
          anchor="left"
        ...
        <AppBar position="static" color="default" title="plotApp">
          <IconButton
            onClick={event => {
              this.toggleCharacterList(event)
            }}
          >
            <HomeIcon />
          </IconButton>
